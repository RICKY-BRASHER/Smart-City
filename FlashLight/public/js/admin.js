...
function toggleDarkMode(isDark) {
    ...
    sessionStorage.setItem('sc-theme', isDark ? 'dark' : 'light');
}

// Initialiser le thème depuis sessionStorage
(function() {
    const saved = sessionStorage.getItem('sc-theme');
    if (saved === 'dark') {
...
if (typeof Chart === 'undefined') {
    console.error('Chart.js is not loaded. Please include it before this script.');
} else {
    Chart.defaults.font.family = "'Plus Jakarta Sans', sans-serif";
    Chart.defaults.plugins.legend.display = false;
}

// Graphique en bandes (bar chart statique)
function initBarLineChart() {
    if (typeof Chart === 'undefined') return;
    const ctx = document.getElementById('barLineChart')?.getContext('2d');
    if (!ctx) return;

    const vals = [18, 22, 15, 30, 28, 40, 35, 22, 18, 25, 42, 38, 30, 20, 26, 34, 28, 45, 38, 50, 42, 36, 28, 22, 40, 35, 30, 48, 52, 44];
...
function initDonutChart() {
    ...
    // Calculer les données dynamiquement à partir de INCIDENTS
    const categories = {};
    INCIDENTS.forEach(inc => {
        categories[inc.cat] = (categories[inc.cat] || 0) + 1;
    });

    const labels = Object.keys(categories).map(cat => CAT_LABEL[cat] || cat);
...
function initBarChart() {
    ...
    // Exemple de données (à remplacer par des données réelles si nécessaire)
    ...
    new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                data: hoursData,
                backgroundColor: hoursData.map((_, i) => (i >= 8 && i <= 10) ? BLUE : 'rgba(26,111,212,.18)'),
                borderRadius: 4,
                borderSkipped: false
            }]
        },
...
// Créer une petite icône pour la mini-carte
...
// Construire une popup pour les incidents
...
// Initialiser la mini-carte (pour le dashboard)
...
function updateMapStats(data) {
    ...
    const statProgEl = document.getElementById('statProg');
    const statResEl = document.getElementById('statRes');
    const statRejEl = document.getElementById('statRej');

    if (statTotalEl) statTotalEl.textContent = data.length;
    if (statNewEl) statNewEl.textContent = c.new;
    if (statProgEl) statProgEl.textContent = c.progress;
    if (statResEl) statResEl.textContent = c.resolved;
    if (statRejEl) statRejEl.textContent = c.rejected;
}

// Appliquer les filtres de la carte
function applyMapFilters() {
    if (!markerCluster || !leafletMap) return;
    ...
    allMarkers.forEach(marker => {
        ...
        if ((!cat || d.cat === cat) &&
            ...
            (!prio || d.prio === prio) &&
            (!zone || d.zone === zone) &&
            (!search || d.title.toLowerCase().includes(search) || d.loc.toLowerCase().includes(search) || d.id.toLowerCase().includes(search))) {
            markerCluster.addLayer(marker);
            visible.push(d);
        }
    });
    ...
    if (visibleCountEl) {
        visibleCountEl.textContent = visible.length + ' incident' + (visible.length > 1 ? 's' : '');
    }

    updateMapStats(visible);

    if (visible.length > 0 && leafletMap) {
...
// Réinitialiser les filtres de la carte
function resetMapFilters() {
    ['mapFilterCat', 'mapFilterStatus', 'mapFilterPrio', 'mapFilterZone'].forEach(id => {
        const el = document.getElementById(id);
        if (el) el.value = '';
    });
    const searchEl = document.getElementById('mapFilterSearch');
    if (searchEl) searchEl.value = '';
    applyMapFilters();
    if (leafletMap) leafletMap.setView([4.0511, 9.7140], 14);
}

// Fonction pour naviguer entre les pages (exemple)
function navigateTo(page) {
    console.log(`Navigation vers : ${page}`);
    // Logique de navigation à implémenter selon votre structure HTML
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Écouteurs pour les filtres de la carte
    ['mapFilterCat', 'mapFilterStatus', 'mapFilterPrio', 'mapFilterZone', 'mapFilterSearch'].forEach(id => {
        const el = document.getElementById(id);
        if (el) {
            el.addEventListener('input', applyMapFilters);
            el.addEventListener('change', applyMapFilters);
        }
    });

    // Écouteur pour le bouton de réinitialisation des filtres
    const resetBtn = document.getElementById('mapFilterReset');
    if (resetBtn) {
        resetBtn.addEventListener('click', resetMapFilters);
    }
